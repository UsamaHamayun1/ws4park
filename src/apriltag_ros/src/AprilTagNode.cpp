...
private:
    ...
    std::atomic<double> min_decision_margin;
    ...
    tf_broadcaster(this)
{
    ...
    declare_parameter("min_decision_margin", 0.0, descr("reject detections with confidence lower than threshold"));
                            ...
                            const sensor_msgs::msg::CameraInfo::ConstSharedPtr& msg_ci)
{
    ...
    for(int i = 0; i < zarray_size(detections); i++) {
        ...
        // reject detections with low confidence
        if(det->decision_margin < min_decision_margin) { continue; }
...
AprilTagNode::onParameter(const std::vector<rclcpp::Parameter>& parameters)
{
    ...
    for(const rclcpp::Parameter& parameter : parameters) {
        ...
        IF("min_decision_margin", min_decision_margin)
        IF("profile", profile)
    }
